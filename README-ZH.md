...
- 登录世界会临时隔离玩家状态，避免主世界游戏模式、背包、装备、副手、经验、药水效果等出现在登录大厅。
- 登录世界状态快照会临时保存到磁盘，服务器更新、重载或崩溃后也能在玩家下次进入时恢复。
- 登录后完整快照恢复可按传送策略配置：回到上次位置默认恢复，回到出生点默认不恢复。
...
- 支持本地桥接通行证，登录服验证成功后签发，主服校验通过才放行玩家。
- 跨服模式下支持通过本地桥接同步 `/rememberlogin on|off` 状态，玩家可在主服开启免登录，登录服下次进服时自动识别。
- 旧版配置会在启动时自动补齐新版本缺失的默认配置项，已有配置值不会被覆盖。
- 玩家可自行使用 `/rememberlogin on|off` 开关短时免登录，默认不开启。
- `/login` 重复输入不会重置密码错误次数，密码锁定倒计时会持续显示。
...
- `login-world-settings.isolate-player-state`：是否隔离登录世界玩家状态，避免登录世界污染主世界背包和游戏模式。
- `login-world-settings.restore-snapshot-at-spawn`：传送到主世界出生点时是否恢复完整玩家快照，默认关闭。
...
- `bridge-verification`：登录服和主服共享的一次性通行证验证。两边必须使用相同目录和相同 `secret`。
...
同一台机器上的 Login 子服和 MC 主服推荐使用 `bridge-verification` 本地桥接目录。跨机器部署时，再考虑把 `storage.type` 改为 `mysql` 或 `mariadb`。

跨服免登录同步依赖 `bridge-verification`。如果玩家在后端主服执行 `/rememberlogin on`，插件会把该玩家的免登录状态写入共享桥接目录；玩家下次进入登录服时，登录服会读取并校验这份状态，从而按配置自动免登录。登录服和后端主服的 `bridge-verification.directory` 与 `bridge-verification.secret` 必须完全一致。
...
在本地桥接多服模式下，后端主服执行 `/rememberlogin on|off` 产生的免登录同步状态会保存到 `bridge-verification.directory` 指向的共享目录中，文件名以 `remember-` 开头，并使用 `bridge-verification.secret` 签名校验。

IP 会用于新的登录环境提醒。插件不会读取玩家设备硬件信息，异常登录判断基于服务端可获得的连接信息。

登录世界状态隔离产生的临时快照存放在：

`plugins/LoginGate/LoginSnapshots/<玩家UUID>.yml`

玩家登录成功或状态恢复后会自动删除对应快照。若服务器异常关闭导致文件残留，玩家下次进服时会优先读取并恢复。
